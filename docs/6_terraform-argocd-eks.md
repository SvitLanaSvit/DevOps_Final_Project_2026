...
Далі я перевірила, що DNS-записи дійсно працюють, за допомогою `nslookup` (у WSL/Ubuntu або на локальній машині):

```bash
nslookup argocd.sk.devops10.test-danit.com
```

Очікуваний результат – у відповіді має бути IP-адреса, яка відповідає AWS ELB/NLB, наприклад:

```text
Non-authoritative answer:
Name:   aab8932e1099f4b3496b3e16a44e6c9c-294ceaf98e4824eb.elb.eu-central-1.amazonaws.com
Address: 3.120.xxx.xxx
Aliases: argocd.sk.devops10.test-danit.com
```

Якщо з’являється `NXDOMAIN` або `No answer`, потрібно трохи почекати або очистити DNS-кеш Windows (див. примітку в розділі 6 нижче).
...
> Примітка про DNS-кеш Windows
>
> Після повного `terraform destroy` і нового `terraform apply` DNS-запис для `argocd.sk.devops10.test-danit.com` у Route53 може вже існувати, але Windows продовжує використовувати старий кешований результат (`NXDOMAIN`). У такому випадку сайт не відкривається, поки не оновиться кеш.
>
> Щоб примусово очистити DNS-кеш на Windows, я виконала в PowerShell/Command Prompt команду:
>
> ```bat
> ipconfig /flushdns
> ```
>
> Після цього наступний запит до `https://argocd.sk.devops10.test-danit.com` пішов до DNS-сервера знову, підтягнув новий запис із Route53, і ArgoCD успішно відкрився.
