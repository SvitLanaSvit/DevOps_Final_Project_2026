...
- **presentation/** – інтерактивна UI‑презентація проєкту (слайди з навігацією):
   - `index.html` – головна сторінка презентації;
   - `styles.css` – оформлення;
   - `script.js` – логіка перемикання слайдів (кнопки, список, клавіші).
...
## UI‑презентація в репозиторії

У проєкті є окремий UI‑розділ для захисту з перемиканням слайдів, швидким переходом на будь-яку сторінку та поверненням назад.

Запуск локально:

1. Відкрий [presentation/index.html](presentation/index.html) у браузері.
2. Перемикання слайдів:
   - кнопки **Назад / Вперед**;
   - клік по потрібному слайду в лівому меню;
